...
echo.
echo [2/2] Starting live tactical simulation...
echo This mode advances authoritative combat state in fixed simulation ticks while the window is open.
echo Controls:
echo   SPACE       pause/resume
echo   N or RIGHT  advance exactly one simulation tick while paused
echo   R           reset deterministic scenario
echo   1..6        speed 0.25x / 0.5x / 1x / 2x / 4x / 8x
echo   F1          toggle debug HUD
echo   ESC         exit
echo.
"%JAVA_EXE%" -jar "%APP_JAR%" --live-tactical-sim
set "APP_EXIT_CODE=%ERRORLEVEL%"

if not "%APP_EXIT_CODE%"=="0" (
    echo.
    echo [ERROR] Live tactical simulation exited with code %APP_EXIT_CODE%.
    pause
)
