...
public final class FleetOrderSubmissionService {
    ...
    private static void validateReadiness(
        ...
        if (readiness.crewBps() <= 0) {
            throw new IllegalStateException("fleet has no observed crew availability: " + fleetId);
        }
        if (type.combatOrder()) {
            if (readiness.structuralBps() < COMBAT_STRUCTURAL_MIN_BPS
        ...
        if (type.serviceOrder() && readiness.supplyAccessBps() <= 0) {
            // Supply access is observed at the force boundary; this prevents a service order from
            // manufacturing an implicit supply chain. Target capability is validated separately.
    ...
    public record ServiceCapability(
        ...
        public static ServiceCapability none() {
            return new ServiceCapability(false, false, false, 0L, 1L);
        }
    }
}
