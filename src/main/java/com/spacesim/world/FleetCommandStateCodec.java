...
public final class FleetCommandStateCodec {
    private static final int MAGIC = 0x46323144; // F21D
    ...
    public static byte[] encode(FleetCommandState state) {
        ...
        try {
            ...
            try (DataOutputStream out = new DataOutputStream(buffer)) {
                ...
                out.writeLong(checked.nextCommandGroupId());
                out.writeLong(checked.nextOrderId());
                ...
                for (CommandGroupState group : checked.groups()) {
                    out.writeLong(group.id());
                    out.writeInt(group.factionId());
                    out.writeUTF(group.name());
                    out.writeLong(group.homeSystemId().value());
                    out.writeBoolean(group.reserve());
                    out.writeBoolean(group.homeDefense());
                ...
                for (FleetOrderState order : checked.orders()) {
                    ...
                    out.writeLong(order.commandGroupId());
                    out.writeUTF(order.type().name());
                    out.writeUTF(order.source().name());
                    out.writeLong(order.targetSystemId().value());
                    out.writeInt(order.routeCursor());
                    out.writeLong(order.submittedTick());
                    out.writeLong(order.stagingDeadlineTick());
            ...
            if (bytes.length <= 0 || bytes.length > MAX_BYTES) {
                throw new IllegalArgumentException("Stage-21D command payload exceeds bounded size");
            }
            return bytes;
    ...
    public static FleetCommandState decode(byte[] bytes) {
        ...
        if (bytes.length <= 0 || bytes.length > MAX_BYTES) {
            throw new IllegalArgumentException("Stage-21D command payload size outside bounds");
        }
        try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
            ...
            long nextGroup = in.readLong();
            long nextOrder = in.readLong();
            ...
            for (int i = 0; i < groupCount; i++) {
                ...
                boolean reserve = in.readBoolean();
                boolean homeDefense = in.readBoolean();
                ...
                groups.add(new CommandGroupState(id, factionId, name, members, home, reserve, homeDefense));
            }
            ...
            for (int i = 0; i < orderCount; i++) {
                ...
                OrderType type = OrderType.valueOf(in.readUTF());
                OrderSource source = OrderSource.valueOf(in.readUTF());
                StarSystemId target = new StarSystemId(in.readLong());
                int cursor = in.readInt();
                long submitted = in.readLong();
                long deadline = in.readLong();
                OrderStatus status = OrderStatus.valueOf(in.readUTF());
