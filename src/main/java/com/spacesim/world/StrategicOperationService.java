...
public final class StrategicOperationService {
    ...
    /**
     * Starts a Stage-21E operation from the command group's already accepted active Stage-21D order.
     *
     * @param state current operation registry
     * @param commandState accepted Stage-21D command/order state
     * @param forces current read-only physical force reconstruction
     * @param commandGroupId group whose active order is being admitted
     * @param currentTick authoritative current tick
     * @param rulesOfEngagement explicit operation ROE
     * @param supplyPolicy explicit physical readiness/supply continuation policy
     * @param withdrawalPolicy explicit ordinary-route withdrawal policy
     * @return immutable state containing the admitted operation
     */
    public StrategicOperationState beginFromActiveOrder(
        ...
        for (FleetId fleetId : group.memberFleetIds()) {
            ...
            if (stagingSystem == null) {
                stagingSystem = force.systemId();
        ...
        if (stagingSystem == null) {
            throw new IllegalStateException("operation command group contains no physical fleets");
        }
        ...
        OperationState operation = new OperationState(
                ...
                order.id(),
                group.factionId(),
                group.memberFleetIds(),
                stagingSystem,
                order.targetSystemId(),
                "system:" + order.targetSystemId().value(),
                rulesOfEngagement,
                supplyPolicy,
                withdrawalPolicy,
                initialStatus,
                ...
                null,
                null);
        return state.add(operation);
    }

    /**
     * Advances STAGING to ACTIVE only when every surviving participant physically occupies the objective system.
     *
     * @param state operation registry
     ...
     * @return updated immutable registry
     */
    public StrategicOperationState activateWhenPhysicallyArrived(
        ...
        if (operation.status() != OperationStatus.STAGING) {
            return state;
        }
    ...
    /**
     * Acquires a target exclusively from an actor-bounded security observation snapshot.
     *
     * <p>The method deliberately receives no WorldSimulation/WorldState argument. The target fleet id
     * must occur as the exact observation target string and its evidence must be current at the
     * supplied tick. Physical truth is checked only later by the tactical materialization bridge.</p>
     *
     * @param state current operation registry
     * @param operationId active operation identity
     * @param observations actor-bounded snapshot already delivered to the operation owner
     * @param targetFleetId target identity reported to the actor
     * @param observedSystemId system named by the delivered report adapter
     * @param currentTick authoritative decision tick
     * @return updated registry retaining the exact report provenance
     */
    public StrategicOperationState acquireContact(
        ...
        ContactState contact = new ContactState(
                ...
                evidence.observedAtTick(),
                evidence.freshUntilTick());
        return state.replace(operation.withLifecycle(
     ...
     * @param encounter validated encounter reference produced by the materialization bridge
     * @param currentTick authoritative tick
     * @return updated operation registry
     */
    ...
    /**
     * Re-evaluates physical readiness and supply access without modifying those physical facts.
     *
     * @param state operation registry
     * @param operationId operation identity
     * @param forces current ordinary force reconstruction
     * @param currentTick authoritative tick
     * @return decision plus immutable operation state reflecting unsupported/withdrawal lifecycle
     */
    public SupplyReview reviewSupplyAndReadiness(
        ...
        if (!operation.status().active()) {
            return new SupplyReview(state, SupplyDecision.CONTINUE);
        }

        boolean missingParticipant = false;
        boolean readinessFailure = false;
        ...
        for (FleetId fleetId : operation.participantFleetIds()) {
            ...
            readinessFailure |= !readiness.missionCapable(operation.supplyPolicy().minimumMissionReadinessBps());
        ...
        boolean unsupported = readinessFailure || supplyFailure;
        long unsupportedSince = unsupported
                ? (operation.unsupportedSinceTick() >= 0L ? operation.unsupportedSinceTick() : currentTick)
                : -1L;
        boolean unsupportedExpired = unsupported && currentTick - unsupportedSince >= operation.supplyPolicy().maximumUnsupportedTicks();
        boolean withdraw = unsupportedExpired
                || readinessFailure
    ...
    public static OperationType operationType(OrderType type) {
        return switch (Objects.requireNonNull(type, "type")) {
            ...
            default -> throw new IllegalArgumentException("Stage-21D order does not materialize a Stage-21E operation: " + type);
        };
    }

    /** Physical continuation decision; withdrawal must be submitted through Stage-21D order authority. */
    public enum SupplyDecision {
        /** Current physical facts permit operation continuation. */ CONTINUE,
        /** Complete/cancel the source order then submit ordinary WITHDRAW through FleetOrderSubmissionService. */ SUBMIT_ORDINARY_WITHDRAW_ORDER,
        /** No participant survived in the ordinary fleet registry. */ FAIL_NO_SURVIVORS,
        /** Surviving force lacks physical propellant required by configured withdrawal policy. */ FAIL_CANNOT_WITHDRAW
    }

    /** @param state updated immutable operation state @param decision required continuation action */
    public record SupplyReview(StrategicOperationState state, SupplyDecision decision) {
        /** Validates supply review result. */
        public SupplyReview {
            Objects.requireNonNull(state, "state");
