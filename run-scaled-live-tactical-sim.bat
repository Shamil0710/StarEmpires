...
echo.
echo [2/2] Starting Stage 19 scaled live tactical simulation...
echo This viewer runs the shared authoritative 32-ship ^(16v16^) Stage 19I saturation scenario.
echo Controls:
...
echo   ESC         exit
echo.
"%JAVA_EXE%" -jar "%APP_JAR%" --scaled-live-tactical-sim
set "APP_EXIT_CODE=%ERRORLEVEL%"

if not "%APP_EXIT_CODE%"=="0" (
    echo.
    echo [ERROR] Stage 19 scaled live tactical simulation exited with code %APP_EXIT_CODE%.
    pause
)
