# Star Empires — Stage 21 Content Width / Technology / Balance / Long-run Stability

> Статус: **PLANNED**  
> Основание: accepted `Ship Mathematics v1.0 Design Baseline`; production Stage 17.5 должен быть завершён до основной content expansion  
> Назначение: расширить мир, корабли, модули, технологии и faction differentiation, не создавая вторую систему правил.

---

# 1. Главный принцип Stage 21

> **Stage 21 расширяет пространство решений внутри v1.0, а не добавляет скрытые parallel stats ради удобства контента.**

Новый hull/module/technology считается допустимым только если его преимущества и недостатки выражаются через accepted budgets/interfaces.
...
До основного Stage 21 должны быть стабильны:

- Stage 17.5 production fitting/combat foundation;
- Stage 18 strategic warfare/tactical AI;
- Stage 19 physically calibrated world generation;
- Stage 20 NPC/missions/reputation mechanics;
- v1.0 schema and capability APIs;
- real construction/refit/repair/maintenance seams.

Stage 21 может получать early content prototypes раньше, но массовая балансировка не должна строиться поверх временных mechanics.
...
# 4. Stage 21A — material/component technology catalog

Расширить baseline material catalog до practical content set.
...
Stage 21 расширяет `HeavyImpactResponseSurface` dataset.
...
# 5. Stage 21B — reactors, energy storage и electrical distribution families
...
# 6. Stage 21C — propulsion / maneuver / reaction-mass / FTL families
...
# 8. Stage 21E — sensors, communications, fire control и EW catalog
...
# 9. Stage 21F — weapon and ammunition families
...
Ammo имеет real manufacturing/resource cost и logistics footprint.

Fleet sustained warfare должен физически потреблять ammunition production.

---

# 10. Stage 21G — shields and protection families
...
# 11. Stage 21H — hull families and variants
...
# 12. Stage 21I — faction engineering doctrines
...
# 13. Stage 21J — shipyard/facility/production ladder

Ship production требует соответствующей industrial capability.
...
# 14. Stage 21K — construction / maintenance / repair / replacement economics
...
# 15. Stage 21L — fleet composition and doctrine balance
...
# 16. Stage 21M — combat saturation / endurance soak

Expand v0.5-style matrices.
...
# 17. Stage 21N — world-scale logistics soak

На Stage-19 generated regions проверять:
...
# 18. Stage 21O — macro economy / long-run simulation soak
...
# 19. Stage 21P — anti-universal-build matrix
...
# 20. Stage 21Q — anti-linear-tier-obsolescence matrix
...
# 21. Stage 21R — faction differentiation acceptance
...
но все factions используют одну физику.

---

# 22. Stage 21S — player progression and market availability
...
# 24. Architecture change policy during Stage 21

Новый content request, требующий поля вне v1.0 contract, проходит вопросы:

1. можно ли выразить capability существующими physical parameters?
2. это новый derived UI metric или authoritative resource?
3. влияет ли он на player и AI одинаково?
4. нужен ли persistence?
5. влияет ли на world/economy scale?
6. требует ли migration?
...
# 25. Stage 21 completion gate

Stage 21 COMPLETE, когда:
...
- high-tier production имеет реальные bottlenecks;
- ammunition/reaction mass/repair logistics работают в long-run;
- world-scale economy стабильна на representative seeds;
...
# 26. Итоговый Stage 21 invariant
...
→ changes fleet doctrine and economic geography
```
