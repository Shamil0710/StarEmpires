# Обновление спрайтов для B19

Ветка включает обновление Империи из PR #357. Запускайте `M22_6_Human_Review_Runner.bat` из обновлённой ветки.

1. Выберите пункт 1 для подготовки старого машинного RC.
2. Выберите пункт 4 для нового пакета B19; отдельная чистая worktree с графикой `2f4215e8` создаётся автоматически.
3. Пункт 5 начинает новую сессию, пункт 6 продолжает её, пункт 7 завершает.

Новые результаты: `Documents\StarEmpires-M22.6-HumanReview-5fb4c523-art-2f4215e8`.
Старые ответы остаются в прежней папке. Не переносите их в новый пакет.
В identity и manifest отдельно записаны `buildSha` машинного RC и `visualBuildSha` графики.
Результат нового B19 относится к новой графике; он не подтверждает прежний замороженный visual fingerprint.
B18/B20 остаются заблокированными. Для закрытия M22.6 требуется согласование нового visual freeze.
